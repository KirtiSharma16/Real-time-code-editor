...
const EditorPage = () => {
  ...
  const { roomId } = useParams();       // getting roomID from url using useParams hook
  ...
  useEffect(() => {
    const init = async () => {
      ...
      function handleErrors(e) {
        ...
        reactNavigator("/");  // redirecting to the home page if connection fails
      }

      socketRef.current.emit(ACTIONS.JOIN, {
        roomId,
        username: location.state?.username,           // extracting username through saved state
      });
      ...
      socketRef.current.on(
        ...
        ({ clients, username, socketId }) => {
          ...
          // sending toast to all the users inside the room except the one which has joined
      ...
      socketRef.current.on(ACTIONS.DISCONNECTED, ({ socketId, username }) => {
        ...
        setClients((prev) => {
          return prev.filter((client) => client.socketId !== socketId);     // updating the clients list 
        });
      });
    ...
    // clearing the listeners to prevent memory leak
    return () => {
  ...
  const runCode = () => {
    ...
    const options = {
      ...
      headers: {
        ...
        'X-RapidAPI-Key': 'e5eda60678msh6274d8c457a9725p17df1fjsn3f4a9ae37009',
    ...
    <div className="mainWrap">
      <div className="asideWrap">
        <div className="asideInner">
          <div className="logo">
            <img className="logoImage" src="/website-logo.png" alt="logo" />
          </div>
          <h3>Connected</h3>
